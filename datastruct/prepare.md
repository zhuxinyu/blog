...
24. 从O(n<sup>c</sup>)到O(2<sup>n</sup>)是从**有效算法**到**无效算法**的分水岭

25. 2-Subset is NP-complete (集合一分为二，求补集问题) ：直觉算法 即 2<sup>n</sup>, 就目前的计算模型而言，**不存在**可在多项式时间内回答此问题的算法，直觉算法已属最优

26. 算法分析的主要任务 = 正确性（不变性 x 单调性） + 复杂度
...
29. 幂方级数：比幂次高出一阶
    ...
    T(n) = 1<sup>2</sup> + 2<sup>2</sup> + ... + n<sup>2</sup> = n(n+1)(2n+1)/6 = O(n<sup>3</sup>)
...
32. 几何分布**(某种意义上，基本操作次数、存储单元可能是分数)**：

    (1 - λ)·[1 + 2λ + 3λ<sup>2</sup> + 4λ<sup>3</sup> + ...] = 1/(1-λ) = O(1), 0 < λ < 1
